/*
 * MusselBedHeater_field_RevC_old.ino
 * This version is intended for use in the field for running
 * experiments (and lab testing as needed), but only with an 
 * old RevC version of the board, which uses a different analog
 * voltage reference than the newer RevF boards. 
 * 
...
// voltmeter and measure at the AVCC and GND pins on the board). Enter the 
// voltage value here. 
float refVoltage = 3.00; // Voltage at RevC AVCC pin on ATmega microcontroller, measured per board
...
//  analogReference(EXTERNAL); // RevC boards just use Vcc as analog reference voltage
...
//          Serial.println(F("Short press registered"));
  ...
  switch(mainState){
    case STATE_IDLE:
      ...
      // No temperature rise is added in STATE_IDLE mode, so the setpoint
    ...
    case STATE_OFF:
        ...
        // Update pidSetpoint value for record-keeping purposes
        // No temperature rise is added in STATE_OFF mode, so the setpoint
        // should equal the reference temperatures.
        pidSetpoint = avgMAXtemp;
...
void writeToSD (DateTime timestamp, double refTemps[]) {
  ...
  logfile.print(F(","));
  logfile.print(batteryVolts,2); // write current battery voltage, 2 decimal places
  logfile.print(F(","));
  logfile.print(tideHeightft,2); // write current predicted tide height, 2 decimal places
  logfile.print(F(","));
  logfile.print(mainState); // Write a number representing what state the program is in. (0 = idle, 1 = heat, 2 = ended)
  logfile.println();
