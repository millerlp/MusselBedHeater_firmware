/*
 * Thermistor_initial_test2.ino
 * Luke Miller 2020
 * 
 * Testing with MusselBedHeater RevF hardware
...
#define NUM_THERMS  1     // Number of thermistors to read

double vREF = 3.000; // Analog voltage reference value (volts)
unsigned int analogRaw;
static double voltageRaw;
double res2;
...
//  uint32_t tick = micros();
  for (byte i = 0; i < NUM_THERMS; i++){
    ...
    delay(1);
    analogRaw = analogRead(ANALOG_INPUT);
    voltageRaw = (analogRaw/1023.0) * vREF;

    double res1 = (1023.0/analogRaw) - 1;
//    double res1 = ((1023.0/analogRaw)*(3.464/vREF)) - 1;
    res2 = 4700.0 / res1;
//
//    double voltageRaw2 = (1023.0/analogRaw)-1;
//    double res3 = 4700.0 / voltageRaw2;
...
//  uint32_t tock = micros();
  for (byte i = 0; i < NUM_THERMS; i++){
    Serial.print(analogRaw); Serial.print("\tVolts: ");
    Serial.print(voltageRaw);Serial.print("\tResistance: ");Serial.print(res2);
//    Serial.print("\tResSimple"); Serial.print(res3);
    Serial.print("\tTherm Temp: ");Serial.print(thermTemps[i]);
//    Serial.print(F(","));
  }
