...
//    Serial.println(F("SD OKAY"));
  ...
  if (saveData){
    ...
    initTuningFileName(sd, logfile, newtime, filename, serialValid, serialNumber, NUM_THERMISTORS, kp, ki, kd, tempIncrease); // generate a file name
    ...
    if ( (newMillis - prevMaxTime) >= MAXSampleTime){
      ...
      if (saveData && writeData){
        ...
        if (oldtime.day() != oldday) {
          ...
          initTuningFileName(sd, logfile, oldtime, filename, serialValid, serialNumber, NUM_THERMISTORS, kp, ki, kd, tempIncrease); // generate a file name
...
//        pwm.sleep(); // Put PCA9685 chip to sleep for now
    ...
    case STATE_OFF:
        ...
        mainState = STATE_OFF;
        saveData = false; // Cancel any data writing to SD card now. 
